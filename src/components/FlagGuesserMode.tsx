...
/** Build per-continent accuracy breakdown from the flags seen/answered this session. */
...
export default function FlagGuesserMode() {
  ...
  // Continent tracking: which country codes were shown / answered correctly.
  ...
  // Focus the input and record the flag as "seen" whenever the current flag changes.
  ...
  const finishQuiz = useCallback((completedAll: boolean) => {
    ...
    saveGameResult({
      ...
      continent_breakdown: computeContinentBreakdown(seenCodesRef.current, correctCodesRef.current),
    });
  }, []);
  ...
  const handleGuessChange = useCallback(
    (value: string) => {
      ...
      if (isCorrect(value, current, countries)) {
        correctCodesRef.current.add(current.code);
